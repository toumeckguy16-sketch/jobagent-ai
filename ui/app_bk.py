...
def init_session():
    ...
    # â”€â”€ Charger le profil depuis Firestore â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            ...
            if "chats_list" in history:
                ...
                for k, v in history.items():
                    if k != "chats_list" and isinstance(v, list):
                        migrated.append({"id": f"Historique : {k}", "messages": v})
...
def _run_search_pipeline(user_profile: str):
    ...
    with st.spinner("Les agents travaillent pour vous..."):
        ...
        # â”€â”€ Sauvegarder dans Firestore â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
with st.sidebar:
    ...
    if st.session_state.candidate_profile:
        ...
        # L'upload ne doit apparaà®tre QUE si aucune photo n'est encore définie
        if not img_url:
...
# ============================================================
    ...
    if st.session_state.previous_page:
        ...
        with col_b:
            if st.button("â†", key="back_analyse_end", help="Retour à la page précédente"):
...
# ============================================================
    ...
    if not st.session_state.selected_job:
            ...
            if not st.session_state.quiz_questions:
                ...
                # Score final
                    ...
                    msg = (
                        ...
                        if pct >= 50 else
                        "Continuez àvous entraà®ner — utilisez le chat pour approfondir."
                    )
        ...
        with subtab2:
            ...
            with col_new:
                if st.button("Commencer un nouvel entretien virtuel", use_container_width=True):
                    with st.spinner("Coach démarre l'entretien..."):
                        if st.session_state.use_mock:
                            welcome = f"Bonjour ! Bienvenue àvotre entretien pour le poste de {job['title']}. Parlez-moi un peu de vous."
                        else:
            ...
            if not active_chat:
                st.markdown(f"<div style='background:#1A1A1A; border:1px dashed #2A2A2A; border-radius:14px; padding:48px; text-align:center; margin-top:16px;'><div style='font-size:1.8em;'>&#128172;</div><div style='font-family:Roboto,sans-serif; font-weight:700; color:#E5E5E5; margin-top:12px;'>Prêt pour votre entretien ?</div><div style='color:#888888; font-size:0.9em; margin-top:8px;'>Cliquez sur <b>Commencer un nouvel entretien virtuel</b> pour démarrer,<br>ou reprenez une conversation depuis l'historique.</div></div>", unsafe_allow_html=True)
            ...
            if active_chat:
                ...
                if prompt := st.chat_input("Posez une question au Coach ou répondez àsa question..."):
    ...
    if st.session_state.previous_page:
        ...
        col_b, _ = st.columns([1, 15])
        with col_b:
            if st.button("â†", key="back_prep_end", help="Retour à la page précédente"):
                st.session_state.current_page = st.session_state.previous_page
                st.session_state.previous_page = None
                st.rerun()
